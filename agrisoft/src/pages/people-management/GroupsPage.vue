        ...
        <DxPopupWidget :visible="isViewPopupVisible" @hiding="isViewPopupVisible = false" :drag-enabled="false"
            ...
            <div class="view-details-container p-6 overflow-y-auto h-full space-y-8 bg-gray-50/50 dark:bg-transparent">
                ...
                <div
                    ...
                    <div
                        class="w-20 h-20 rounded-2xl bg-teal-600 flex items-center justify-center text-white text-3xl font-black shadow-lg shadow-teal-600/20">
                        {{ String(viewRecordData.name || 'G').charAt(0) }}
                    </div>
                    <div class="flex flex-col">
                ...
                <div class="px-2">
                    <div class="detail-section">
                        <h3
                            class="text-xs font-black text-teal-600 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                            <span class="w-8 h-[1px] bg-teal-600/30"></span> Grupo de Trabajo
                        </h3>
                        <div
...
const dataSource = new CustomStore({
    ...
    insert: async (values) => {
        ...
        try {
            ...
            const res = await conexionApi.post('/management-people/groups/createGroup', payload)
    ...
    update: async (key, values) => {
        ...
        try {
            ...
            const res = await conexionApi.post('/management-people/groups/updateGroup', payload)
            if (res.data && res.data.code === 'ERROR') {
    ...
    remove: async (key) => {
        loading.value = true
        try {
            const res = await conexionApi.post('/management-people/groups/deleteGroup', { id: key })
            if (res.data && res.data.code === 'ERROR') {
